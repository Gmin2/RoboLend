#!/usr/bin/env bash
# liquidation.sh — Liquidation flow smoke test
#   deposit → borrow → crash price → liquidate → cleanup
#
# Usage:
#   bash script/liquidation.sh
...
echo "========================================="
echo "  Liquidation Flow Smoke Test"
echo "========================================="
...
# ── Step 3: Deposit 5 TSLA + borrow 0.0005 WETH ──
echo ""
echo "=== Step 3: Deposit 5 TSLA + borrow 0.0005 WETH ==="
...
echo ""
echo "=== Step 4: Crash TSLA price \$250 -> \$100 ==="
cast send "$ORACLE" "setPrice(address,uint256)" "$TSLA" 10000000000 $C > /dev/null

HF_AFTER=$(cast call "$POOL" "getHealthFactor(address)(uint256)" "$DEPLOYER" --rpc-url "$RPC")
echo "  Health factor after crash: $HF_AFTER"
echo "  Position should be underwater (HF < 1e18)"

# ── Step 5: Liquidate 25% of debt (self-liquidation) ──
echo ""
echo "=== Step 5: Liquidate 25% of debt ==="
DEBT_BEFORE=$(cast call "$POOL" "getUserDebt(address)(uint256)" "$DEPLOYER" --rpc-url "$RPC" | awk '{print $1}')
echo "  Debt before: $DEBT_BEFORE"

# 25% of debt
DEBT_TO_REPAY=$((DEBT_BEFORE / 4))
echo "  Liquidating: $DEBT_TO_REPAY"

cast send "$WETH" "approve(address,uint256)" "$POOL" "$DEBT_TO_REPAY" $C > /dev/null
cast send "$POOL" "liquidate(address,address,uint256)" "$DEPLOYER" "$TSLA" "$DEBT_TO_REPAY" $C > /dev/null

DEBT_AFTER=$(cast call "$POOL" "getUserDebt(address)(uint256)" "$DEPLOYER" --rpc-url "$RPC")
echo "  Debt after liquidation: $DEBT_AFTER"
echo "  Remaining collateral: $(cast call "$POOL" "getUserCollateral(address,address)(uint256)" "$DEPLOYER" "$TSLA" --rpc-url "$RPC")"

# ── Step 6: Cleanup — restore price, repay, withdraw ──
echo ""
echo "=== Step 6: Cleanup ==="

# Restore TSLA price to $250
cast send "$ORACLE" "setPrice(address,uint256)" "$TSLA" 25000000000 $C > /dev/null
echo "  TSLA price restored to \$250"

# Repay remaining debt (approve with buffer to cover interest accrual)
REMAINING_DEBT=$(cast call "$POOL" "getUserDebt(address)(uint256)" "$DEPLOYER" --rpc-url "$RPC" | awk '{print $1}')
if [ "$REMAINING_DEBT" != "0" ]; then
    echo "  Repaying remaining debt: $REMAINING_DEBT"
...
REMAINING_COLLATERAL=$(cast call "$POOL" "getUserCollateral(address,address)(uint256)" "$DEPLOYER" "$TSLA" --rpc-url "$RPC" | awk '{print $1}')
if [ "$REMAINING_COLLATERAL" != "0" ]; then
    echo "  Withdrawing remaining collateral: $REMAINING_COLLATERAL"
    cast send "$POOL" "withdraw(address,uint256)" "$TSLA" "$REMAINING_COLLATERAL" $C > /dev/null
fi
...
echo ""
echo "========================================="
echo "  Liquidation Flow Complete!"
echo "========================================="
